...
async fn close_active_connections(
  ...
  let closed_conn_ids = futures::future::join_all(close_futs).await;

  // Record all closed connections with the coordinator so the shutdown state machine
  // can advance. For connections backed by a real actor (e.g. SCA/TCP sessions), the
  // actor's ActorStopping event will also call record_connection_closed, but that is
  // idempotent (a second remove on a missing key is a no-op). For actor-less connections
  // (e.g. UdpSendConnection) this is the only place the coordinator is notified.
  if !closed_conn_ids.is_empty() {
    let mut coordinator = core_arc.shutdown_coordinator.lock().await;
    let mut made_progress_to_linger = false;
    for conn_id_val in closed_conn_ids {
      if coordinator.record_connection_closed(conn_id_val, core_handle) {
    ...
    if made_progress_to_linger && coordinator.state == ShutdownPhase::Lingering {
      tracing::debug!(
        handle = core_handle,
        "close_active_connections: All connections closed. Advancing to linger/cleaning."
      );
      let linger_opt_val = core_arc.core_state.read().options.linger;
      coordinator.start_linger_if_needed(linger_opt_val, core_handle);
    }
  }
}
