...
static PARSED_MSG_TX_FOR_WORKER_TO_PROCESSOR: OnceCell<
  Mutex<Option<SyncSender<(RawFd, HandlerUpstreamEvent)>>>,
> = OnceCell::new();
static PARSED_MSG_RX_FOR_PROCESSOR: OnceCell<
  Mutex<Option<AsyncReceiver<(RawFd, HandlerUpstreamEvent)>>>,
> = OnceCell::new();
static URING_UPSTREAM_PROCESSOR_JOIN_HANDLE: OnceCell<Mutex<Option<TokioTaskJoinHandle<()>>>> =
  OnceCell::new();
...
#[doc(hidden)]
#[cfg(feature = "io-uring")]
pub(crate) fn get_global_parsed_msg_tx_mutex()
-> &'static Mutex<Option<SyncSender<(RawFd, HandlerUpstreamEvent)>>> {
  PARSED_MSG_TX_FOR_WORKER_TO_PROCESSOR.get_or_init(Default::default)
}
#[doc(hidden)]
#[cfg(feature = "io-uring")]
pub(crate) fn get_global_parsed_msg_rx_mutex()
-> &'static Mutex<Option<AsyncReceiver<(RawFd, HandlerUpstreamEvent)>>> {
  PARSED_MSG_RX_FOR_PROCESSOR.get_or_init(Default::default)
}
#[doc(hidden)]
#[cfg(feature = "io-uring")]
pub(crate) fn get_uring_upstream_processor_join_handle_mutex()
-> &'static Mutex<Option<TokioTaskJoinHandle<()>>> {
...
pub(crate) fn ensure_global_uring_systems_started() -> Result<(), ZmqError> {
  if !URING_BACKEND_INITIALIZED.load(std::sync::atomic::Ordering::SeqCst) {
    info!(
      "GlobalUringState: io_uring backend not yet initialized by user. Initializing with default configuration."
    );
...
pub(crate) fn register_uring_fd_socket_core_mailbox(
  fd: RawFd,
  core_mailbox: SocketCoreMailboxSender,
) {
  // Use get_or_init for the OnceCell itself, ensuring it contains the Arc<RwLock<...>>
  let map_arc =
    URING_FD_TO_SOCKET_CORE_MAILBOX_MAP.get_or_init(|| Arc::new(RwLock::new(HashMap::new())));
  debug!(
    raw_fd = fd,
...
pub(crate) fn unregister_uring_fd_socket_core_mailbox(fd: RawFd) {
  if let Some(map_arc) = URING_FD_TO_SOCKET_CORE_MAILBOX_MAP.get() {
    ...
    if map_arc.write().remove(&fd).is_some() {
      trace!(
        raw_fd = fd,
        "GlobalUringState: Successfully unregistered uring FD."
      );
    } else {
      trace!(
        raw_fd = fd,
